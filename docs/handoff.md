...
**その後、ユーザーからの直接指示で新機能2件を `Workflow` ツールで並列実装中。
まだ完了していない（バックグラウンドで実行中、完了通知待ち）。** 詳細は次節「次にやること」。
...
- **`Workflow` を起動した直後にセッションが途切れると、完了通知を誰も受け取れない。**
  バックグラウンド実行中に別のセッション（次の `/checkin` 等）が同じ `task_id` の状態を
  確認しないと、`running` のまま気づかれずに放置される。**`Workflow` を使って未完了の
  まま終える場合、`task_id` と run ID を必ずこの引継ぎに書くこと**（今回は
  `wteslmezj` / `wf_3bc4e6c1-610`）
- **`ひらがな・カタカナのような新しい書き順データを追加させるときは「想像で座標を
  作らない」を明記すること。** 既存の漢字データは KanjiVG（実在のオープンデータ）から
  取得したものだが、AI に「同じ形式で追加して」とだけ頼むと、実在しないデータに対して
  もっともらしい SVG パスを捏造しかねない。学習者に誤った書き順を教えることになるため、
  取得できなければ実装せず報告させる、という指示を明示的に書く必要がある
