...
**`app/`（Senbon 本体）の意味欄の穴を4言語ぶん全部潰し、`main` へマージし終えた。次の作業は決まっていない。**
...
**意味欄の穴（4言語ぶん）は全部解消済み。`docs/neglected-log.md` の Gate `021` の件は閉じている。**
...
- **`play/+page.svelte` には `EN_MEANING` / `ZH_MEANING` / `KO_MEANING` という表がある**（基本漢字
  約95字ぶん）。データ側のキーを消すと、この表 → 英語の順にフォールバックする。
